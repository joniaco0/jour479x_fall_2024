...
```{r}
Teams <- c("ARI", "LAD", "NYY", "BAL", "MIL", "PHI", "SDP", "HOU", "NYM", "MIN", "TOR", "CHC", "CLE", "SEA", "BOS", "KCR", "DET", "ATL", "SFG", "TEX", "STL", "TBR", "OAK", "WSN", "CIN", "PIT", "MIA", "LAA", "COL", "CHW","FLA","TBD")
...
```{r}
standings <- read_csv("C:/desktop/jour479x_fall_2024/Stand.csv")
View(Standings)
...
```
I want to do this for other players, but this is the base formatting. 

```{r}
ss <- SweetSpot |> select(Name)
names <- data.frame(ss)
combined |> filter(filter(Name %in% names$Name)) |> group_by(Name)
```
